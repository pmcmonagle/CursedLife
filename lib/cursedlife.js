...
function CursedLife() {
    ...
    // Send any lowercase letter (or space) to the input window. This is an awkward
    // way of doing things in Blessed, and is mostly left over from how
    // the old node-ncurses library functioned.
    this.main.key('abcdefghijklmnopqrstuvwxyz '.split(''), (function (ch, key) {
        this.input.onInputChar(ch, key);
    }).bind(this));

    // Send the backspace key to the input window as well.
    this.main.key(['backspace'], (function (ch, key) {
        this.input.onInputChar(ch, key);
    }).bind(this));
    ...
    // The enter key gets sent to output window only if the input window
    // has no string waiting to be activated!
    ...
    */
    ...
    // Quit cleanly
    this.main.key(['escape', 'q', 'C-c'], function (ch, key) {
